      ...
      <section className="relative pt-section pb-xl px-lg hero-bg text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto space-y-5">
          ...
          <p className="text-body-lg text-on-surface-variant max-w-2xl mx-auto">
            We're here to help. Reach out through any channel and our team will respond promptly.
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-xl">
          ...
          <div className="space-y-6">
            <div className="glass-panel rounded-xl p-6 space-y-4">
              <h2 className="font-heading text-xl font-semibold text-on-surface">Contact Information</h2>
              {[
                ...
                <div key={item.label} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold tracking-widest uppercase text-on-surface-variant">{item.label}</p>
                    {item.href ? (
                      <a href={item.href} target="_blank" rel="noopener noreferrer" className="text-sm text-on-surface hover:text-primary transition-colors">{item.value}</a>
                    ) : (
            ...
            {/* Social Links */}
            <div className="glass-panel rounded-xl p-6 space-y-4">
              <h3 className="font-heading font-semibold text-on-surface">Follow Us</h3>
              <div className="flex gap-3">
                {[
                  ...
                  <a
                    ...
                    href={s.href}
                    aria-label={s.label}
                    className="w-10 h-10 rounded-full bg-surface-container flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-primary/10 transition-colors border border-outline-variant/30"
                  >
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-400 text-white font-semibold py-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-green-500/30"
            >
          ...
          <div className="lg:col-span-2">
            <div className="glass-panel rounded-xl p-8">
              <h2 className="font-heading text-2xl font-semibold text-on-surface mb-6">Send Us a Message</h2>

              {success ? (
                <div className="flex flex-col items-center gap-4 py-12 text-center">
                  <span className="material-symbols-outlined text-5xl text-primary">check_circle</span>
                  <h3 className="font-heading text-xl font-semibold text-on-surface">Message Sent!</h3>
                  <p className="text-on-surface-variant">We'll get back to you within 24 hours.</p>
                  <button onClick={() => setSuccess(false)} className="text-primary hover:underline text-sm">Send another message</button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-semibold tracking-widest uppercase text-on-surface-variant mb-2">Full Name *</label>
                      <input
                        type="text"
                        name="fullName"
                        value={form.fullName}
                    ...
                    <div>
                      <label className="block text-xs font-semibold tracking-widest uppercase text-on-surface-variant mb-2">Phone / WhatsApp</label>
                      <input
                        type="tel"
                        name="phone"
                        value={form.phone}
                    ...
                    <div>
                      <label className="block text-xs font-semibold tracking-widest uppercase text-on-surface-variant mb-2">Country</label>
                      <input
                        type="text"
                        name="country"
                        value={form.country}
                  ...
                  <div>
                    <label className="block text-xs font-semibold tracking-widest uppercase text-on-surface-variant mb-2">Preferred Service</label>
                    <select
                      name="service"
                      value={form.service}
                      onChange={handleChange}
                  ...
                  <div>
                    <label className="block text-xs font-semibold tracking-widest uppercase text-on-surface-variant mb-2">Message *</label>
                    <textarea
                      name="message"
                      value={form.message}
                      onChange={handleChange}
                      ...
                      className="w-full bg-surface-container border border-outline-variant/30 rounded-lg px-4 py-3 text-sm text-on-surface placeholder-on-surface-variant/50 focus:outline-none focus:border-primary transition-colors resize-none"
                    />
                  </div>
