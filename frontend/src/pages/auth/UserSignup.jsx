...
const COUNTRIES = ['Afghanistan', 'Pakistan', 'Iran', 'Germany', 'United Kingdom', 'USA', 'Canada', 'Australia', 'Turkey', 'China', 'Other']
const INTERESTS = [
...
export default function UserSignup() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* ── Left Panel ── */}
      <div className="hidden lg:flex lg:w-5/12 hero-bg flex-col justify-between p-12 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-80 h-80 bg-primary/20 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl pointer-events-none" />
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="space-y-4">
            {[
              ...
              { icon: 'check_circle', text: 'Apply to scholarships with one click' },
              { icon: 'check_circle', text: 'Track all your applications in one place' },
              { icon: 'check_circle', text: 'Get personalized scholarship recommendations' },
              { icon: 'check_circle', text: 'Receive announcements and new opportunities' },
            ].map((b) => (
              <div key={b.text} className="flex items-center gap-3">
                <span className="material-symbols-outlined text-primary text-xl flex-shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>{b.icon}</span>
                <span className="text-primary-light/90 text-sm">{b.text}</span>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={s.label} className="bg-white/10 border border-white/20 rounded-xl p-3 text-center">
                <div className="font-heading font-bold text-white text-xl">{s.value}</div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-12 bg-background overflow-y-auto">
        <div className="w-full max-w-lg mx-auto">
          ...
          {/* Header */}
          <div className="mb-8">
            <h1 className="font-heading font-bold text-navy text-3xl mb-2">Create your account</h1>
            ...
            {STEPS.map((label, i) => (
              <React.Fragment key={label}>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs font-medium hidden sm:block ${i === step ? 'text-primary' : 'text-text-muted'}`}>
                    {label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showConfirm ? 'text' : 'password'} name="confirmPassword" value={form.confirmPassword} onChange={handleChange} required
                      placeholder="Re-enter your password" className={`input-field pl-11 pr-11 ${
